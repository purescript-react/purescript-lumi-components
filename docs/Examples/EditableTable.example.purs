        ...
        [ example $
            editableTable
              ...
              , columns:
                  ...
                  , { label: "Lead time"
                    , renderCell: \row -> Input.input Input.number
                        { value = show row.leadTime
                        , onChange = handler targetValue \value ->
                            updateRow self row { leadTime = fromMaybe row.leadTime (Int.fromNumber =<< (readInt 10 <$> value)) }
                        }
                    }
                  , { label: "Actual cost"
                    , renderCell: \row -> Input.input Input.number
                        { value = show row.actualCost
                        , onChange = handler targetValue \value ->
                            updateRow self $ fromMaybe row do
                              value' <- readInt 10 <$> value
                              pure if isNaN value'
                                then row
                                else row { actualCost = value' }
                        }
                    }
                  , { label: "Price"
                    , renderCell: \row -> Input.input Input.number
                  ...
                  , { label: "Margin"
                    , renderCell: \row -> Input.input Input.number
                        { value = show row.margin
                        , onChange = handler targetValue \value ->
                            updateRow self $ fromMaybe row do
                              value' <- readInt 10 <$> value
                              pure if isNaN value'
                                then row
                                else row { margin = value' }
                        }
                    }
                  , { label: "Total cost"
